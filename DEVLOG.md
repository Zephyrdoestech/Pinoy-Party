...
**Last Updated:** 2026-06-30 (Gameplay sync: dice rolls, turn order, player count, and minigame launch are now host-authoritative and tested working over LAN with 2 players; in-minigame input is NOT yet synced, see Known Issues)
...
> **Known gap, not yet addressed:** the sync above stops at "which minigame loads, for whom, and when." Once inside, `LuksongBaka`/`SackRace`/`LangitLupa` all still read raw local `Input`/`InputEvent` per client with no networking at all — a keypress on one window does nothing on any other window's copy of the same minigame instance. Each client is simulating its own independent playthrough simultaneously. This is the next major piece of work; see Known Issues & TODOs.
...
- **In-minigame input not networked (LAN, 2026-06-30)** — minigame *selection and launch* are now host-synced (see LAN Multiplayer Lobby → Gameplay Sync), but once inside `LuksongBaka`/`SackRace`/`LangitLupa`, each client still reads raw local keyboard input with zero RPC traffic. A keypress on one window has no effect on any other window's copy of the minigame — each client simulates its own independent playthrough. This is the next major piece of multiplayer work, not yet started.
...
- **LAN multiplayer — lobby + core turn loop synced, in-minigame input NOT yet synced (2026-06-30).** Hosting/joining/roster-sync, player count, turn order, dice rolls, movement (works "for free" via deterministic board + synced rolls), and minigame selection/launch are all host-authoritative and verified working over a live 2-window LAN test. See "Gameplay Sync" under LAN Multiplayer Lobby for the full design. What's left: once a minigame scene loads, each client still simulates it from fully local keyboard input with zero networking — `LangitLupa`'s `local_player_index` pattern (currently hardcoded to `0`) is the intended template for this, anticipating each LAN client eventually only controlling its own player within a minigame, same shape as the request→host-broadcast pattern already used for dice rolls.
...
| `LangitLupa` | ✅ Implemented, in rotation | Real-time tag with elevated safe-zones; confirmed loading/scoring correctly via LAN minigame-launch sync (2026-06-30). In-minigame movement input still not networked — see Known Issues. |
