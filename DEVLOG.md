...
**Last Updated:** 2026-06-30 (Gameplay sync: dice rolls, turn order, player count, minigame launch, SackRace, and LuksongBaka are now host-authoritative and tested working over LAN with 2 players; LangitLupa in-minigame input is NOT yet synced, see Known Issues)
...
> **Known gap, not yet addressed:** the sync above stops at "which minigame loads, for whom, and when." Once inside, `LangitLupa` still reads raw local `Input`/`InputEvent` per client with no networking at all — a movement keypress on one window does nothing on any other window's copy. `LuksongBaka` and `SackRace` are now fully synced (see Recent Bug Fixes). `LangitLupa` is the remaining piece; it needs a different sync approach than the other two since it uses continuous WASD movement rather than discrete button presses — see Known Issues & TODOs.
...
- **In-minigame input not networked — LangitLupa only (LAN, 2026-06-30)** — `SackRace` and `LuksongBaka` are now fully synced. `LangitLupa` still reads raw local keyboard input with zero RPC traffic — movement keypresses on one window have no effect on any other window's copy. Needs a different sync approach than the other two (continuous WASD movement rather than discrete presses) — not yet started.
...
- **LAN multiplayer — lobby + core turn loop + SackRace + LuksongBaka synced; LangitLupa in-minigame input NOT yet synced (2026-06-30).** Hosting/joining/roster-sync, player count, turn order, dice rolls, movement, minigame selection/launch, and in-minigame input for `SackRace` and `LuksongBaka` are all host-authoritative and verified working over live 2-window LAN testing. See "Gameplay Sync" under LAN Multiplayer Lobby for the full design. What's left: `LangitLupa`'s continuous WASD movement input — needs a periodic position-broadcast approach rather than the discrete-press request/broadcast pattern used for the other two minigames.
